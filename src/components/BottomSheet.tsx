...
const BottomSheet: React.FC<BottomSheetProps> = ({ isOpen, onClose, children, bgColor, title }) => {
  return (
    <div
      className={`fixed inset-x-0 bottom-[10vh] transform ${isOpen ? 'translate-y-0' : 'translate-y-full'} transition-transform duration-300 ease-in-out z-30 ${bgColor ? `bg-[${bgColor}]` : 'bg-[#000000]'} rounded-tl-lg rounded-tr-lg w-[95%] mx-auto overflow-y-scroll pb-15`}
      style={{ height: '85vh' }}
    >
